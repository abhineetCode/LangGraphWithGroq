...
while True:
  ...
  for event in events:
    print(event["messages"][-1].additional_kwargs)
    event["messages"][-1].pretty_print()

    #check if any tools is about to be called
    if "tool_calls" in event["messages"][-1].additional_kwargs:
        print("Current State: ", graph.get_state(config).values)

        tool_name = event["messages"][-1].additional_kwargs["tool_calls"][0]["function"]["name"]
        tool_args = event["messages"][-1].additional_kwargs["tool_calls"][0]["function"]["arguments"]
        ...
        if tool_name == "human_assistance":
            ...
            pass
# I need some some expert assistance for building and AI agent.Could you request assistance for me?
# WE, the experts are here to help! we would recommend you check out Langgraph to build your agent. it's much more reliable and extensible then simple autonomous agents.
